...
│   │   ├── generate-assets.mjs       # icon/splash generation
...
- **Localisation (Phase 3a)**: a dependency-free typed i18n layer in `mobile/src/lib/i18n.ts` with
  `en` (source of truth) + `hi` dictionaries — **311 keys** (286 in 3a + 25 in 3b), key-set equality enforced at compile
...
- **Voice + haptics feedback (Phase 3b)**: `expo-speech` + `expo-haptics` (both
  `~57.0.2`, permission-free, Expo-Go compatible — no `app.json` change, no dev
  build). **One** module owns the event→feedback mapping
  (`src/features/crew/crew-feedback.ts`); integration points call only
  `feedback.on('board.done', …)`. The spoken channel is **Latin-script Hinglish**
  (`voice.*` keys) because budget Android devices often ship no `hi-IN` TTS voice
  and a Devanagari string read by the default English voice is garbled — the
  _written_ UI stays Devanagari, they are two channels. Throttled (≤6
  announcements per 10 s burst, latest-wins interrupt, a "5 students boarded"
  summary instead of 40 phrases, **no queue**), privacy-guarded (first name +
  clock time only; phone/medical/guardian/admission strings are refused), and
  role-defaulted (crew voice+vibration on, admin/parent voice off) with two
  toggles on the Help & support screen persisted under `sbt.mobile.feedback`.
  Details, the measured contrast table and the guards: `docs/mobile-ux.md` →
  Phase 3b; support triage: `docs/mobile-operations.md` → Voice/Vibration.
...
15. **Feedback is dispatched from one crew module; speech is never awaited and never fails an
    action.** Every spoken/vibration confirmation goes through `feedback.on(event)` in
    `mobile/src/features/crew/crew-feedback.ts` — never by calling `expo-speech`/`expo-haptics`
    from a screen. The dispatch path contains no `await` (spec-asserted), a native failure is
    swallowed, and a disabled channel makes **zero** native calls. Voice payloads carry a first
    name and a clock time only: phone numbers, medical notes, guardian contacts, emergency detail
    text and admission numbers are refused by a deny-list checked on the final string.
...
| [`docs/mobile-operations.md`](./docs/mobile-operations.md)                                         | Offline attendance, background GPS, session/network UX, 403 taxonomy, **language / voice / vibration settings and their support triage**, build config                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [`docs/mobile-expo-sdk.md`](./docs/mobile-expo-sdk.md)                                             | Expo SDK pinning policy, verified version matrix, Expo Go vs dev builds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [`docs/mobile-ux.md`](./docs/mobile-ux.md)                                                         | Mobile legibility system (Phase 1) + Phase 2 crew screens: status→colour card, hold-to-confirm SOS, full-row board/drop, Help/Support telemetry move — measured contrast tables + guard specs. **+ Phase 3a localisation**: 311-key `en`/`hi` dictionaries, resolution order, the server-string boundary, clipping budgets, grep gate. **+ Phase 3b voice + haptics**: the feedback matrix, why the spoken channel is Latin-script Hinglish while the screen stays Devanagari, the throttle policy, the non-blocking rule, the voice privacy deny-list, and what was deliberately left alone |
