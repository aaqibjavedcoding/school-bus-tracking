...
"""Exports platform-shaped versions of the rendered square masters.
  ...
  videos/16x9/<id>.mp4   1920x1080  LinkedIn desktop, YouTube, website (K11 by default)
...
Usage:
  ...
  python3 export_versions.py --subdir=hinglish   # the Hinglish re-narrations
"""
...
def main() -> None:
    ...
    subdir = None
    for a in argv:
        if a.startswith("--subdir="):
            subdir = a.split("=", 1)[1]
    src_dir = os.path.join(VIDEO_DIR, subdir) if subdir else VIDEO_DIR
    out_root = src_dir if subdir else VIDEO_DIR
    srcs = sorted(glob.glob(os.path.join(src_dir, "*.mp4")))
    if args:
    ...
    if "--vertical-only" not in flags:
        landscape = srcs if "--all" in flags else [
            s for s in srcs if os.path.basename(s).replace("HI_", "").startswith(HERO)
        ]
    else:
        landscape = []
    ...
    for s in srcs:
        ...
        export(s, os.path.join(out_root, "9x16"), VERTICAL, V_CRF)
    for s in landscape:
        print(f"[16:9] {os.path.basename(s)}")
        export(s, os.path.join(out_root, "16x9"), HORIZONTAL, H_CRF)
