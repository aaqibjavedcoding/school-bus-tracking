...
export function createRateLimitStore(store: string, logger = new Logger('RateLimit')): RateLimitStore {
  if (store === 'redis') {
  ...
  providers: [
    {
      ...
      useFactory: (configService: ConfigService) =>
        createRateLimitStore(configService.get<string>('rateLimit.store', 'memory')),
    },
