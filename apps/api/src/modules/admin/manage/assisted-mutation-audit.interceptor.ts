 ...
 */
@Injectable()
export class AssistedMutationAuditInterceptor implements NestInterceptor {
  constructor(
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const request = http.getRequest<ManagedRequest>();
    ...
    return next.handle().pipe(
      tap((payload) => {
        void this.record(
          request,
          managedSchool as ManagedSchoolContext,
          payload as SuccessEnvelope,
        );
      }),
    );
  }
