...
describe('TripsController authorization', () => {
  ...
  it('allows an admin and rejects every other authenticated role with 403', async () => {
    ...
    for (const role of [
      ...
      await assert.rejects(
        activateGuards(request, updateStatusHandler),
  ...
  it('passes only the JWT school id to every trip service call', async () => {
    ...
    const service = {
      ...
      findAll: async (schoolId: string, query: ListTripsQueryDto) => {
      ...
      updateStatus: async (schoolId: string, id: string, dto: UpdateTripStatusDto) => {
        calls.push({ method: 'updateStatus', schoolId, id, dto });
        return { id };
      },
    ...
    await controller.findAll(SCHOOL_B, new ListTripsQueryDto());
    await controller.findOne(SCHOOL_B, TRIP_ID);
    await controller.update(SCHOOL_B, TRIP_ID, new UpdateTripDto());
    await controller.updateStatus(SCHOOL_B, TRIP_ID, status);
