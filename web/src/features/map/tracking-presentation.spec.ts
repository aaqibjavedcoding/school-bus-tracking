...
/**
 * Honest live / stale / offline presentation — the web mirror of
 * `mobile/src/features/map/tracking-presentation.spec.ts`, pinning the same
 * thresholds from the same shared constants.
 *
...
describe('single definition of "live"', () => {
  it('the web map resolves the same shared windows the native map does', () => {
    // `GPS_LIVE_WINDOW_MS` / `GPS_STALE_WINDOW_MS` come from
    // `@school-bus-tracking/shared-types` — the same module the mobile map and
    // the crew controller resolve theirs from, so web and native cannot drift.
    assert.equal(LIVE_WINDOW_MS, 30_000);
    assert.equal(STALE_WINDOW_MS, 120_000);
  });
...
describe('weak accuracy', () => {
  ...
  it('treats exactly-at-threshold accuracy as precise, not approximate', () => {
    const result = deriveTrackingPresentation({
      fixAgeMs: 4_000,
