...
/**
 * Parent-only notification bell content.
 *
 * Kept in its own module (imported lazily by `NotificationBell`) so the
 * realtime stack behind `useParentNotifications` — `socket.io-client` and
 * the `/notifications` socket module — is only downloaded for browsers whose
 * session actually renders a parent shell. School admin / crew / driver
 * sessions never import this file.
 *
 * Shows the live unread count; opening it reveals the most recent
 * notifications. New events arrive over the `/notifications` socket and pop
 * a toast immediately — no refresh. Clicking a notification marks it read
 * and jumps to the relevant child (or tracking page).
 */
export const ParentNotificationBell: React.FC = () => {
  ...
  const { unreadCount, recent, loading, markRead, onNew } = useParentNotifications({
    recentLimit: 8,
    // This component only renders for parent sessions; the hook stays on its
    // safe default (enabled) since the outer bell already gates the role.
    enabled: true,
  });
